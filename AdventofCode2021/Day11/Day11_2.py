...
all_flashed = False
count = sum([len(j) for j in mapa])

total_prev = 0
cycles = 0

while not all_flashed:
    mapa, total_next = one_step(mapa, total_prev)
    cycles += 1
    if cycles % 50 == 0:
        print(f"Flashed: {total_prev}")
        print(f"Cycle: {cycles}, Flashed: {total_next-total_prev}")
    if total_next-total_prev == count:
        print(f"Flashed: {total_prev}")
        print(f"Cycle: {cycles}, Flashed: {total_next-total_prev}")
        all_flashed = True
